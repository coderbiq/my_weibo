...
{ 
    ...
    //广播大厅时间线
    function public_timeline($format='json',$pos=0,$reqnum=20) 
    { 
    	$param['format']=$format;
    	$param['pos']=$pos;
    	$param['reqnum']=$reqnum;
	...
	//首页时间线
    function home_timeline($format='json',$pageflag=0,$pagetime=0,$reqnum=20) 
    { 
    	$param['format']=$format;
	...
	//收听某个人
    function add_idol($format='json',$name='') 
    { 
        $param['name'] =$name; 
        $param['format']=$format;
        return $this->oauth->post( 'http://open.t.qq.com/api/friends/add' ,$param); 
    } 
    function addtweet($format='json',$content='') 
    { 
    ...
    { 
  	...
  	//发表带图片微博
    function addtweet_pic($format='json',$content='',$pic_data='') 
    { 
        $param['format']=$format;
        $param['pic']=$pic_data;
        $param['content']=$content;
